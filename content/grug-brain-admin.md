---
title: "The Grug Brained Admin"
menu: "main"
weight: 2
---

> grug brain admin still writing

# Introduction

this collection of thoughts on hosting software or ap gathered by grug brain admin

grug brain admin not so smart, but grug brain admin host many long year and learn some things
although mostly still confused

grug brain admin try collect learns into small, easily digestible and funny page, not only for you, the young grug, but also for him
because as grug brain admin get older he forget important things, like what had for breakfast or if put pants on

big brained admins are many, and some not expected to like this, make sour face

sometimes big brained admins have other name with fancy acro... shortcut, like Site reliability engineer (SRE), DevOps(grug no dev, grub script), platform engineer, systems engineer, systems architect, cloud engineer

*THINK* they are big brained admins many, many more, and more even definitely probably maybe not like this, many
...
complexity is spirit demon that enter infrastructure through well-meaning but ultimately very clubbable non grug-brain
admins and project managers who not fear complexity spirit demon or even know about sometime

one day infrastructure understandable and grug can get work done, everything good!

next day impossible: complexity demon spirit has entered infrastructure and very dangerous situation!  

grug no able see complexity demon, but grug sense presence in infrastructure and network

demon complexity spirit mocking him make change here break unrelated thing there what!?! mock mock mock ha ha so funny 
grug love server and not becoming shiney rock speculator like grug senior advise

club not work on demon spirit complexity and bad idea actually hit admin who let spirit in with club: sometimes grug himself!
...
"no, grug not use kuberknetes"

"no, grug not build cluster for database"
...
80/20 solution say "80 want with 20 time"  solution maybe not have all bell-whistle that project manager want, maybe a 
...
some grug brain admins like hardware, you can turn off hardware with club

other grug brain admins like cloud, can not see, therefore no club switch

both okay

grug needs to be careful with cloud, cloud often expensive
big cloud expensive and better for many grug brain admin, big cloud have api, but big cloud needs degree on cost understanding

small cloud also good, good for small grug brain admins


## Use old good tools

- not use fresh tools
- if tools decades old use, if tool new shiny wait
  - like with club, club must feel well, new club might hit different, you may hit to hard, or worse to soft when club needed,
  - but look at new club and tools, sometimes they solve problems, but also you get new unknown 
  - no google with new tool, grug then needs look code, no thank

## linux

grug like ubuntu server, ubuntu friendly easy friend

sometimes

## container

grug like container, best container is lunch container, but docker container also good

before lunch container, grug bring bread, sausage, butter in hand, now grug can put in container, no messy hands

docker container same, before grug install software on ubuntu, when install many software, sometimes linux breaks

grug learned the hard way, dependency daemon comes and changes things with time, grug no change and then update kaputt

now grug use docker container, grug also show lunch container to his friend grug brain developer, he now happy with easy dependency installation

## infrastructure as code

sometimes grug had DevOps as title, grug not understand, grug brain admin also do work of friend grug brain developer

grug brain likes infra as code, but only if automation worth

infra code hard, infra code take time, be careful how much you need

many tools for infra, use what all grug brain admins get, ansible, terraform, puppet, chef, salt

## clusters are bad

new tools sometimes good, but ask if tool is cluster

cluster is food for complexity daemon

cluster promise wrong, often cluster says, high available, no downtime, automatic, but grug knows best

when cluster kaputt, no grug brain admin and big brain admin know what wrong
...
when 100% is goal by project manager just not do it
project manager no idea about admin things
you can say not possible and use 80/20

grug does not know phone number of grug, grug not know grug wife number
but grug remembers funny image name+number: xkdc 1319 automation
![xkdc 1319 automation](https://imgs.xkcd.com/comics/automation.png)

grug always need to check automation image
tasks sometimes no need to automate if done not often

grug does not know chicken egg, or egg chicken
same with automation, really really hard
grug need to look at time, when doing 1000 then yes, when doing 10 then no, no silver club

## security

grug touch server, other admin grug touch server, better grug brain admin, not big brain admins with fancy name

grug make simple, grug use fire
...
grug note many such impostor feels in admin duty

always grug one of two states: grug is ruler of all survey, wield code server like thor OR grug have no idea what doing
